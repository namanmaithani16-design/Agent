...
def send_event(action, screenshot_path=None, username=None, metadata=None):
    ...
    if screenshot_path:
        try:
            ...
            print(f"⚠️ [API] Could not read screenshot: {e}")
    ...
    if current_user:
        ...
        print("⚠️ [API] No active session found when sending event")
        username = "unknown"
        email = None
        domain = None
        designation = None
    ...
    # Only fetch session row for login/logout
    session_row = None
    if action in {"login", "logout"} and username != "unknown":
        try:
            session_row = get_open_session_activity(username)
        except Exception as e:
            print(f"⚠️ [API] Could not fetch session row: {e}")

    # Start with base metadata then merge extra metadata
    base_metadata = {
